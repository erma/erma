...
/**
 * A monitor that holds related submonitors. This can be useful for gathering
 * multiple monitors and correlating them with some parent monitor.<p>
 *
 * Children of CompositeMonitors will implicitly inherit their parent's
 * inheritable attributes. CompositeMonitors can designate attributes as
 * inheritable by using the <code>setInheritable</code> methods. Attributes are
 * <strong>not</strong> inheritable when using the <code>set</code> methods.
 *
 * @author Doug Barth
 */
public interface CompositeMonitor extends Monitor {
    /**
     * Adds a child monitor to this composite monitor. The interface makes no
     * prescription as to whether the children are ordered.
     *
     ...
     *
     * @return a collection of child monitors that were captured. This returns
     *         an empty collection if none were set.
     */
    ...
    /**
     * Gets those attributes that can be inherited by the child monitors. These
     * attributes will be added to the child monitor when it is created.
     *
     * @return a reference to the map of the inheritable attributes that should
     *         be added to the child monitors
     */
